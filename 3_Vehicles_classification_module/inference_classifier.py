...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        help="The path to the classification model",
        default="model/model_ResNet50.h5",
    )
    parser.add_argument(
        "--img_size",
        help="The size of the images in length*width",
        default="224*224"
    )
    parser.add_argument(
        "--data",
        help="The path to the image crops generated by the tracking module",
        default="./video/tracking/",
    )
    parser.add_argument(
        "--max_img_in_memory",
        help="The number of croped images loaded in memory at the same time",
        default=32,
    )
    ...
    for chunk in chunks:
        ...
        for index, row in enumerate(chunk.iterrows()):
            ...
            image = tf.keras.preprocessing.image.load_img(
                image_name,
                color_mode="rgb",
                target_size=(int(length), int(width))
            )
        ...
        chunk.to_csv("inference_results.csv",
                     mode="a",
                     header=False,
                     index=False)
        ...
        vehicle_df = pd.concat(
            [
                ...
                pd.DataFrame(
                    [
                        [
                            current_vehicle_df["path"].iloc[0],
